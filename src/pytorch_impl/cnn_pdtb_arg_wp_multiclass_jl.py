...
class PDTB_Classifier(nn.Module):
    ...
    def __init__(self, args):
        ...
        # convolutional layers for wp
        self.conv_layers_wp = nn.ModuleList([nn.Conv2d(1, args.nfmaps_wp, (K, emb_dim), stride = (2, 1)) for K in args.fsz_wp])
        ...
        # initialize conv_layers_wp
        for conv_layer in self.conv_layers_wp:
...
#         # dense layers for wp
#         dense_layers_wp = []
#         for i, D in enumerate(args.dsz_wp):
#             if i == 0:
#                 dense_layers_wp.append(nn.Linear(len(args.fsz_wp) * args.nfmaps_wp, D))
#             else:
#                 dense_layers_wp.append(nn.Linear(args.dsz_wp[i - 1], D))
#
#         self.dense_layers_wp = nn.ModuleList(dense_layers_wp)
#
#         # initialize dense_layers_wp
#         for dense_layer in self.dense_layers_wp:
...
#             # nn.init.zeros_(dense_layer.bias)
        ...
        # define gate2
        self.dense_wp_cap = nn.Linear(2 * len(args.fsz_wp) * args.nfmaps_wp + args.gate_units_arg, args.gate_units_wp)
        # nn.init.xavier_uniform_(self.dense_wp_cap.weight, gain = nn.init.calculate_gain('relu'))
        nn.init.xavier_uniform(self.dense_wp_cap.weight, gain = nn.init.calculate_gain('relu'))
        self.dense_wp_cap.bias.data.fill_(0)

        self.dense_wp_gate = nn.Linear(2 * len(args.fsz_wp) * args.nfmaps_wp + args.gate_units_arg, args.gate_units_wp)
        # nn.init.xavier_uniform_(self.dense_wp_gate.weight, gain = 1)
        nn.init.xavier_uniform(self.dense_wp_gate.weight, gain = 1)
        self.dense_wp_gate.bias.data.fill_(0)

        # classification layer for imp
        self.clf_layer_imp = nn.Linear(args.gate_units_wp, args.nclasses)
        ...
        self.clf_layer_exp = nn.Linear(args.gate_units_wp, args.nclasses)
    ...
    def forward(self, X):
        ...
        X_larg, X_lpos, X_rarg, X_rpos, X_wp, X_wp_pos, X_wp_rev, X_wp_rev_pos, is_imp = X

        h_wp_vecs = []
        for x in [(X_wp, X_wp_pos), (X_wp_rev, X_wp_rev_pos)]:
            ...
            # print ('x_w_p size: ', x_w_p.size())

            x_convs = [F.relu(conv_layer(x_w_p)).squeeze(3) for conv_layer in self.conv_layers_wp]

            # At this point x_convs is [(batch_size, nfmaps_wp, seq_len_new), ...]*len(fsz_wp)
            x_max_pools = [F.max_pool1d(xi, xi.size(2)).squeeze(2) for xi in x_convs]  # [(batch_size, nfmaps_wp), ...]*len(fsz_wp)

            x_max_pool = torch.cat(x_max_pools, 1)

            x_max_pool = nn.Dropout(self.args.dropout_p)(x_max_pool)

            h_wp_vecs.append(x_max_pool)
        ...
        h_arg = h_arg_cap * h_arg_gate

        h_wp_vec = torch.cat(h_wp_vecs, 1)
        h_final = torch.cat([h_arg, h_wp_vec], 1)
        h_final_cap = F.relu(self.dense_wp_cap(h_final))
        h_final_gate = torch.sigmoid(self.dense_wp_gate(h_final))
        h_clf_in = h_final_cap * h_final_gate
...
def test(test_set, args, run_id):
    ...
    print('############################### IMP ###############################')
    print(classification_report(y_true_test_imp, y_pred_test_imp, target_names = args.class_names, digits = 4))
    print('############################### EXP ###############################')
    print(classification_report(y_true_test_exp, y_pred_test_exp, target_names = args.class_names, digits = 4))
...
def main():
    ...
    val_set = [X_val_larg, X_val_lpos, X_val_rarg, X_val_rpos, X_val_wp, X_val_wp_pos, X_val_wp_rev, X_val_wp_rev_pos, is_imp_val, Y_val_all]
    ...
    args.nfmaps_arg = 50
    args.fsz_wp = filter_hs_wp
    args.nfmaps_wp = 100
    args.dsz_arg = []
    args.dsz_wp = []
    args.gate_units_arg = 300
    args.gate_units_wp = 300
